...
public interface WalletRepository extends JpaRepository<WalletEntity, Integer> {
    ...
    List<WalletEntity> findAllByProfileId(int profileId, Pageable pageable);

    @Query("SELECT wallet FROM WalletEntity wallet WHERE wallet.id = :id")
    WalletEntity findById(int id);

    @Query("SELECT wallet FROM WalletEntity wallet WHERE wallet.profile.user.id = :userId")
    WalletEntity findByUserId(int userId);
