...
public interface TransactionRepository extends JpaRepository<TransactionEntity, Integer> {
    ...
    Page<TransactionEntity> findAllByWalletId(int walletId, Pageable pageable);

    @Query("SELECT t FROM TransactionEntity t where t.payeeWallet.id = :walletId AND t.status = :transactionStatus")
    Page<TransactionEntity> findAllByWalletIdAndTransactionStatus(int walletId, TransactionStatus transactionStatus,
                                                                  Pageable pageable);
}
